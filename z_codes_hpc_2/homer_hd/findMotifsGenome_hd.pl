#!/usr/bin/env perl
use warnings;
use lib "/opt/applications/homer/4.9.1/gnu//bin";
my $homeDir = "/opt/applications/homer/4.9.1/gnu/";
...
if ($cmd->{'find'} eq '') {
	...
	if ($cmd->{'noknown'}==1) { #|| ($cmd->{'force'} == 0 && exists($files{$knownMotifHTML}))) {
		...
		`/gpfs/home/hdiao/jycATAC/GEO_ATAC/codes/homer_hd/findKnownMotifs_hd.pl -s "$seqFile" -g "$groupFile" -o "$cmd->{'output'}" -pvalue $knownPvalueThresh -m "$cmd->{'mknown'}" $options`;
	}
